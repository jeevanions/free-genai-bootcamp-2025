...
func main() {
	...
	settingsRepo := repository.NewSQLiteSettingsRepository(db)
	analyticsRepo := repository.NewSQLiteAnalyticsRepository(db)
	dashboardRepo := repository.NewSQLiteDashboardRepository(db)
	...
	dashboardService := services.NewDashboardService(dashboardRepo, sessionRepo, reviewRepo)
	...
	// Setup API v1 routes
	v1 := r.Group("/api/v1")
	router.SetupRoutes(v1, wordService, groupService, sessionService, reviewService, activityService, dashboardService, settingsService, analyticsService)
