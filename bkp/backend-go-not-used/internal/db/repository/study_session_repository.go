...
func (r *SQLiteStudySessionRepository) Create(ctx context.Context, session *models.StudySession) error {
	query := `
		INSERT INTO study_sessions (
			group_id, study_activity_id, total_words, correct_words, 
...
func (r *SQLiteStudySessionRepository) List(ctx context.Context, page, limit int) ([]*models.StudySession, int, error) {
	// Calculate total count
	var total int
	err := r.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM study_sessions").Scan(&total)
	if err != nil {
		return nil, 0, err
	}
