...
func TestWordReviewAPI(t *testing.T) {
	...
	t.Run("Get Word Stats", func(t *testing.T) {
		// Create additional incorrect review
		incorrectReview := models.CreateWordReviewRequest{
			WordID:         word.ID,
			StudySessionID: session.ID,
			Correct:        false,
		}
		body, _ := json.Marshal(incorrectReview)
		req := httptest.NewRequest("POST", "/api/v1/reviews/", bytes.NewBuffer(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
		...
		assert.Equal(t, float64(50), stats.Accuracy) // (1 correct / 2 total) * 100
	})
}
...
func createTestSession(t *testing.T, router *gin.Engine, groupID, activityID int64) *models.StudySessionResponse {
